...
var commit = function(message, options){
    options = options || "";
    return new Promise(function(resolve, reject){
        execFile('git', ['commit', ...options.split(' '), '-m', message], function(err){
            if(err){
                reject(err);
            }
            else {
...
var getCurrentBranch = function(){
    return new Promise(function(resolve, reject){
        execFile('git', ['rev-parse', '--abbrev-ref', 'HEAD'], function(err, result){
            if(err){
                reject(err);
            }
            else {
...
var showFilesAdded = function(){
    return new Promise(function(resolve, reject){
        execFile('git', ['status'], function(err, result){
            if(err){
                reject(err);
            }
            else {
...
var getFilesCached = function(){
    return new Promise(function(resolve, reject){
        execFile('git', ['diff', '--name-only', '--cached'], function(err, result){
            if (err){
                reject(err);
            }
            else {
...
var showFilesModified = function(){
    return new Promise(function(resolve, reject){
        execFile('git', ['diff', '--name-only'], function(err, result){
            if(err){
                reject(err);
            }
            else {
...
var revert = function(files){
    ...
    return new Promise(function(resolve, reject){
        async.each(files, function(file, next){
            execFile('git', ['checkout', '--', file], function(err){
                if (err){
                    next(err);
                }
                else {
...
var getBranches = {
    local: function(){ 
        return new Promise(function(resolve, reject){
            execFile('git', ['show-ref'], function(err, result){
                if(err){
                    reject(err);
                }
                else {
    ...
    all: function(){
        return new Promise(function(resolve, reject){
            execFile('git', ['branch', '-a'], function(err, result){
                if(err){
                    reject(err);
                }
                else {
...
var checkout = function(branch){
    return new Promise(function(resolve, reject){
        execFile('git', ['checkout', branch], function(err, result){
            if(err){
                reject(err);
            }
            else {
...
var newBranch = function(newBranchName){
    return new Promise(function(resolve, reject){
        execFile('git', ['checkout', '-b', newBranchName], function(err){
            if(err){
                reject(err);
            }
            else {
...
var deleteBranch = function(branch){
    return new Promise(function(resolve, reject){
        execFile('git', ['branch', '-d', branch], function(err){
            if (err){
                reject(err);
            }
            else {
